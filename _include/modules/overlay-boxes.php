...
function show_overlay_boxes() {
	...
	foreach ($_boxes as $box) {
		...
		$box['checkout-url']=str_replace("_license","", $box['checkout-url']);
		
		/*if(getActualPage()=='es'){
			$box['checkout-url']=str_replace("_license","", $box['checkout-url']);
			$newurl=explode("/", $box['checkout-url']);
			$box['checkout-url']=$newurl[0]."//".$newurl[2]."/".$newurl[3]."/licencia_".$newurl[4];
		}*/
...
//echo $plan->lc_url_basename;
		...
		$box['checkout-url']=str_replace("_vps","", $box['checkout-url']);
		$box['checkout-url']=str_replace("_dedicated","", $box['checkout-url']);
		$box['checkout-url']=str_replace("_license","", $box['checkout-url']);
		/*if(getActualPage()=='es'){
			$newurl=explode("/", $box['checkout-url']);
			$box['checkout-url']=$newurl[0]."//".$newurl[2]."/".$newurl[3]."/licencia_".$newurl[4];
			...
			$box['checkout-url']=$box['checkout-url']."_"._("license");*/
		?>
		<li onclick="location.href='<?=$box['info-url']?>'" onmouseover="<?=$onmouseover?>" onmouseout="<?=$onmouseout?>">
			...
			<?php } else { ?>
				<!--<img src="<?=$img_dir?>/buttons/<?=$order_now?>" width="116" height="33" alt="<?=$alt_value?>" <?=$style?>/>-->
                 ...
                 <a  rel="nofollow" href="javascript: return false;" style="margin-right:auto; margin-right:2px; margin-left:auto; padding:0 15px 0 0; <?php if(getActualPage()=='es') echo "padding:0 10px 0 0; margin-right:2px; ";?>" class="button-grey"><?=_('Unavailable')?><span <?php if(getActualPage()=='es') echo "style='padding-right:2px !important; padding-left:8px !important'";else "padding-right:17px";?> ></span></a>
