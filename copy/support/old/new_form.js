...
{  		
	$.ajax({
		type: "POST",
		url: "../_include/ajax/FormMail.php",
...
function formSubmitChange() {  	
	function validateNewLicenseConfig() {
		var options = { };

		$.each($('form#change-license select'), function (i, sel) {
			var orig = $(sel).attr('name');
			var name = orig.replace(/^genericOption\[(.*)\]/, '$1');
			if (name != orig && $('option:selected', sel).html() != null) {
				options[name] = $('option:selected', sel).html();
			}
			//if (/genericOption\[(.*)\]/.exec($(sel).attr('name')
		});

		var reqv = [];

		var req = {};
		req.code = $('#license-type option:selected').attr('code');
		req.options = JSON.stringify(options);
		req.app = 'ParallelsHelper';
		req.action = 'validate-config';

		$.ajax({
			url: 'https://my.licensecube.com/mvg/?jsoncallback=sendChangeForm',
			data: req,
			dataType: 'jsonp',
			success: function(data, textStatus, jqXHR) {
				if (data.error) {
					...
					$('#tabc2 .message').html(data.message);
...
function resultChangeLicense(json)
{
	...
	if(json.status=='OK' )
	{
		...
		$('#tabc2 .loader').css('display', 'none');
		$('#tabc2 .button-submit').css('display', '');
		$('#tr-current-ip-1').css('display', 'none');
		$('#tr-new-ip-1').css('display', 'none');
		$('#tr-current-domain-1').css('display', 'none');
		$('#tr-new-domain').css('display', 'none');
		$('#tr-license-id-1').css('display', 'none');
		...
		$.ajax({
			...
			data: { email: $('#email-1').val(), password: $('#password-2').val() },
			dataType: "jsonp",
			success: function(json) {
...
function resultCencelLicense(json)
{
	...
	if(json.status=='OK')
	{
		...
		$('#tabc3 #tr-current-ip-2').css('display', 'none');
		$('#tabc3 #tr-current-domain-2').css('display', 'none');
		$('#tabc3 #new-ip').val('');
		$('#tabc3 #new-domain').val('');

		if ($('#license-type-2 option[id]').length == 1) { // this was his last license
			setTimeout(function() { $('#tabc3 .login-message').css('display', 'none'); }, 15000);
			var first = $('#license-type-2 option[id]').attr('first');
			...
			$.ajax({
				type: "POST",
				url: "https://my.licensecube.com/lc-api/license_info.php",
				data: { email: $('#tabc3 #email-2').val(), password: $('#tabc3 #password-2').val() },
				dataType: "jsonp",
				success: function(json) { 
...
function resultLoginLicense(json)
{
	...
	if(json.status=='OK')
	{
		if($('#login-form-3').val() == 'installation')
		{ 
			$('#login .message-login').css('display', 'none');
			$('#login').fadeOut();
			...
			$('#form-billing-system #lc_sess_id-4').val(json.lc_sess_id);	
			$('#form-control-panel #lc_sess_id-5').val(json.lc_sess_id);	
			$('#form-additional-software #lc_sess_id-6').val(json.lc_sess_id);
			...
			json.packv = packv;
			var SelectHTML = '<select class="lc-input required"  name="license-type" id="license-type-3" onchange="selectInstallation();" ><option selected="selected" disabled="disabled" value=""> Please Select...</option>';		
			...
			$('#tabc4 #type-of-license-3').html(SelectHTML);
			
			$('#table-type-license-id').fadeIn();	
					
			
			$('#form-additional-software #tr-type-license-id-3').css('display', '');
			$('#form-additional-software #tr-type-license-id-3').fadeIn();
			
			$('#form-billing-system #tr-type-license-id-3').css('display', '');
			$('#form-billing-system #tr-type-license-id-3').fadeIn();	
...
{ 
		type = $("#table-type-license-id #license-type-3 option:selected").attr('type')
		id = $("#table-type-license-id #license-type-3 option:selected").attr('id')
		title = $("#table-type-license-id #license-type-3 option:selected").attr('title')
		licenseType = $("#table-type-license-id #license-type-3 option:selected").attr('licenseType')
		ipDomain = $("#table-type-license-id #license-type-3 option:selected").attr('ipDomain')
		ip_address = $("#table-type-license-id #license-type-3 option:selected").attr('ip_address')
		servername = $("#table-type-license-id #license-type-3 option:selected").attr('servername')
...
{ 
	...
	if(type == 'billing_system')
	{
		...
		$('#form-billing-system #pack_id-4').val(id);	
		$("#form-billing-system #ftp_domain-1").attr("value", servername); 
	}
	else if(type == 'control_panel')
	{
		...
		$('#form-control-panel').fadeIn();	
		$('#form-control-panel #pack_id-5').val(id);	
		$("#form-control-panel #server_ip_address-1").attr("value", ip_address); 
	}
	else
	{
		$('#form-control-panel').css('display', 'none');
		...
		$('#form-additional-software').fadeIn();
		$('#form-additional-software #pack_id-6').val(id);
		if (ip_address != '')
		{	
			$("#form-additional-software #server_ip_address-2").attr("value", ip_address); 
			$('#form-additional-software #server_ip_address-2').attr('disabled', 'disabled');
		}
		if (servername != '')
		{
			$("#form-additional-software #ftp_domain-2").attr("value", servername); 
			$('#form-additional-software #ftp_domain-2').attr('disabled', 'disabled');
		}
	}
...
function resultLoginLicense1(json)
{
	...
	if(json.status=='OK')
	{
		if($('#login-form-1').val() == 'installation')
		{ 
			$('#login1 .message-login').css('display', 'none');
			$('#login1').fadeOut();
			...
			$('#change-license #lc_sess_id-1').val(json.lc_sess_id);	
			$('#form-ticket #lc_sess_id-1').val(json.lc_sess_id);	
			...
			if (json.packv.length==1 && json.packv[0].active == 3)
			{
				...
				$('#form-ticket #pack_id-1').val(json.packv[0].id);
				$('#tabc2 #button-ticket').css('display', '');
			...
			else
			{
				...
				var SelectHTML = $('<select/>').
					attr('class',"lc-input required").
					attr('name', "license-type").
				       	attr('id', "license-type").
					attr('onchange', "selectChange();");
					...
					attr('disabled', "disabled").
				       	attr('value', "").
					html('Please Select...'));
				//var SelectHTML = '<select class="lc-input required"  name="license-type" id="license-type-1" onchange="selectChange();" ><option selected="selected" disabled="disabled" value=""> Please Select...</option>';		
				var options = {};
				for(i=0;i<json.packv.length;i++)
				{
					...
					option = $('<option/>').
						attr('value',i).
						attr('requires_ip', json.packv[i].lc_requires_ip).
						attr('requires_license_id', json.packv[i].lc_requires_license_id).
						attr('requires_domain', json.packv[i].lc_requires_domain).
						attr('id', json.packv[i].id).
						attr('current', ipDomainSend).
						attr('title', json.packv[i].title).
						attr('license_id', json.packv[i].lc_license_id).
						attr('active', json.packv[i].active).
						attr('first', json.first).
						attr('last', json.last).
						attr('slaves', json.packv[i].slaves).
						attr('microslaves', json.packv[i].microslaves).
						attr('minislaves', json.packv[i].minislaves).
						attr('code', json.packv[i].code).
						attr('plan_option_groups', JSON.stringify(json.packv[i].plan_option_groups)).
						attr('service_option_groups', JSON.stringify(json.packv[i].service_option_groups)).
						attr('options_editable', json.packv[i].options_editable);

					if (disabled)
						option.attr('disabled', 'disabled');

					option.html(json.packv[i].title + ' ' + ipDomain);
...
//				SelectHTML+='</select>';
				...
				for (var i in options) {
					var optg = $('<optgroup/>').attr('label', i);

					for (var iopt in options[i])
						optg.append(options[i][iopt]);

					SelectHTML.append(optg);
				}
				
				$('#type-of-license-1').html(SelectHTML);
				
				$('#change-license').fadeIn();	
				$('#tr-type-license-id-1').fadeIn();
	...
	$.each($('#change-license tr'), function(i, tr) {
		if (/^tr-/.test($(tr).attr('id')) && ($(tr).css('display') == 'none'))
			originally_hidden.push(tr);
	});
...
function selectChange()
{
		...
		$.each(originally_hidden, function(i,tr) {
			$(tr).css('display', 'none');
		}); 

		requires_ip = $("#tabc2 #license-type option:selected").attr('requires_ip');
		requires_license_id = $("#tabc2 #license-type option:selected").attr('requires_license_id');
		...
		plan_option_groups = JSON.parse($("#license-type option:selected").attr('plan_option_groups'));
		service_option_groups = JSON.parse($("#license-type option:selected").attr('service_option_groups'));
		options_editable = $("#license-type option:selected").attr('options_editable');
		...
		else
		{
			...
			showFormChange(requires_ip, requires_license_id, requires_domain, id, current, title, license_id, slaves, microslaves, minislaves, code, plan_option_groups, service_option_groups, options_editable);
		}
}

function showFormChange(requires_ip, requires_license_id, requires_domain, id, current, title, license_id, slaves, microslaves, minislaves, code, plan_option_groups, service_option_groups, options_editable){
	
	
	
	$('#client-license-change .lc-box').slideUp();
	//$('#change-license').fadeIn();	
	$('#change-license #pack_id-1').val(id);
	$('#tabc2 #title-license').css('display', '');	
	$('#tabc2 #title-license').html(title);
	$('#tabc2 #current-license').css('display', '');
	
	if (requires_license_id == 0)
	{
		$("#tabc2 #tr-license-id-1").css('display', 'none');	
		$("#tabc2 #license-id-1").removeClass("required");
		$("#tabc2 #license-id-1").removeClass("licenseID");
		$('#tabc2 #license-id-1-1').val("");
		
			
	}
	else
	{
		$("#tabc2 #tr-license-id-1").css('display', '');	
		$("#tabc2 #license-id-1").addClass("required");
		
		$("#tabc2 #license-id-1").attr("value", license_id);
	}
	if (requires_ip == 0)
	{
		$("#tabc2 #tr-new-ip-1").css('display', 'none'); 
		$("#tabc2 #tr-current-ip-1").css('display', 'none'); 		
		$("#tabc2 #new-ip-1").removeClass("required");		
		$("#tabc2 #new-ip-1").removeClass("lc-required-missing");		
		$("#tabc2 #new-ip-1").removeClass("ip-address");
		$('#tabc2 #new-ip-1').val("");	
		
		$("#tabc2 #current-ip-1").removeClass("required");		
		$("#tabc2 #current-ip-1").removeClass("lc-required-missing");		
		$("#tabc2 #current-ip-1").removeClass("ip-address");
		$('#tabc2 #current-ip-1').val("");	
		
	}
	else
	{
		$("#tabc2 #tr-new-ip-1").css('display', '');
		$("#tabc2 #tr-current-ip-1").css('display', '');
		$("#tabc2 #new-ip-1").addClass("required");
		
		$("#tabc2 #current-ip-1").addClass("required");
		$("#tabc2 #current-ip-1").attr("value", current); 
	...
	if (requires_domain == 0)
	{
		...
		$("#tabc2 #tr-current-domain-1").css('display', 'none');
		$("#tabc2 #new-domain").removeClass("required");
		$("#tabc2 #new-domain").removeClass("domain");	
		$('#tabc2 #new-domain').val("");
		
		$("#tabc2 #current-domain-1").removeClass("required");
		$("#tabc2 #current-domain-1").removeClass("domain");	
		$('#tabc2 #current-domain-1').val("");
	}
	else
	{
		$("#tabc2 #tr-new-domain").css('display', '');
		$("#tabc2 #tr-current-domain-1").css('display', '');
		$("#tabc2 #new-domain").addClass("required");
		
		$("#tabc2 #current-domain-1").addClass("required");
		$("#tabc2 #current-domain-1").attr("value", current);
	}
	...
	if (jsonLicInfo.packv[license_type].current_os != "") {
		loadComboBox($('#new-os'), jsonLicInfo.packv[license_type].list_os, jsonLicInfo.packv[license_type].current_os);
	...
	if (options_editable)
		show_editable_option(plan_option_groups, service_option_groups);
	
	$('#optionGroup-Containers').change(function(){
		var containers = $('#optionGroup-Containers option:selected').text();
		
		for (var optgid in plan_option_groups) {
			if (plan_option_groups[optgid].title == "Physical Processors") {
				for (var optid in plan_option_groups[optgid].options) {
					var opt = plan_option_groups[optgid].options[optid];
					var title = opt.title;
					var re = new RegExp('Quad \/ '+containers);
					var matches = re.exec(title);
					if (matches != null) {
						$('#optionGroup-PhysicalProcessors option:contains("Quad")').
...
function show_editable_option(plan_option_groups, service_option_groups) {
	// get container.
	var containers = null;
	for (var optgid in plan_option_groups) {
		if (plan_option_groups[optgid].title == "Containers") {
			containers = plan_option_groups[optgid].options[service_option_groups[optgid]].title;
			break;
		}
	}
	...
	for (var optgid in plan_option_groups) {
		var optgtitle = plan_option_groups[optgid].title;
		var trsel = '#tabc2 #tr-' + optgtitle.toLowerCase().replace(/ +/, '-').replace(/[^-_a-z0-9]/, '-');
		var selectsel = '#tabc2 #optionGroup-' + optgtitle.replace(/ +/g, '').replace(/[^-_a-z0-9]+/i, '-');
		$(trsel).css('display','');
		// check if select should be enabled?
		$(selectsel).empty(); // we fill the options
		var optv = [];
		var has_none = false;
		//var is_none = plan_option_groups[optgid].options[service_option_groups[optgid]].title == 'None';
		var prev_optv = []; // we only upgrade, so we keep a list of previous options
		var other_optv = []; // other options, but the one we have selected now.
		var found = false;
		...
		for (var optid in plan_option_groups[optgid].options) {
			...
			if (optgtitle == 'Physical Processors') {
				var re = new RegExp('Dual|Quad \/ '+containers);
				var matches = re.exec(title);
				if (matches != null) {
			...
			if (service_option_groups[optgid] == optid) {
				xopt.attr('selected',  'selected');
				found = true;
			} else {
				other_optv.push(xopt);
			}

			if (!found)
				prev_optv.push(xopt);

			if (!has_none && (opt.title == 'None'))
				has_none = true;
		...
		$.each(optv, function(i, opt) { all_numbers = all_numbers && /^([0-9.]+|Unlimited)(\s+.+)?/.test($(opt).html()); });

		$.each(optv, function(i, opt) { $(selectsel).append(opt); });

		// if (!has_none && !all_numbers)
			// $.each(other_optv, function(i, opt) { $(opt).attr('disabled', 'disabled'); });
		// else
			// $.each(prev_optv, function(i, opt) { $(opt).attr('disabled', 'disabled'); });

		//$(selectsel).change(revalidateLicenseConfig);
	//      var opt = plan_option_groups[optg];
	//      if (plan_option_groups[optg].options[opt
	}
}
...
function resultLoginLicense2(json)
{
	...
	if(json.status=='OK')
	{
		if($('#login-form-2').val() == 'installation')
		{ 
			$('#login2 .message-login').css('display', 'none');
			$('#login2').fadeOut();
			
			$('#cancel-license #lc_sess_id-2').val(json.lc_sess_id);	
			...
			var packv = packv.sort(ordenar); 	
			json.packv = packv;
			var SelectHTML = '<select class="lc-input required"  name="license-type" id="license-type-2" onchange="selectCancel();" ><option selected="selected" disabled="disabled" value=""> Please Select...</option>';		
			...
			{ 
				...
				{	var pro = json.packv[i].lc_product_type;
     			...
     			option='<option requires_ip="' + json.packv[i].lc_requires_ip +'"  requires_license_id="' + json.packv[i].lc_requires_license_id +'" requires_domain="' + json.packv[i].lc_requires_domain +'" id="' + json.packv[i].id +'" current="' + ipDomainSend +'" first="' + json.first + '" title="' + json.packv[i].title +'">' + json.packv[i].title+' '+ ipDomain + '</option>';
     			 SelectHTML+=option;	
			...
			$('#tabc3 #type-of-license-2').html(SelectHTML);
			//$("#tabc3 #license-type").append(SelectHTML);
			
			$('#cancel-license').fadeIn();	
			$('#tabc3 #tr-type-license-id-2').css('display', '');
			$('#tabc3 #tr-type-license-id-2').fadeIn();	
...
{
		requires_ip = $("#tabc3 #license-type-2 option:selected").attr('requires_ip')
		requires_license_id = $("#tabc3 #license-type-2 option:selected").attr('requires_license_id')
		requires_domain = $("#tabc3 #license-type-2 option:selected").attr('requires_domain')
		id = $("#tabc3 #license-type-2 option:selected").attr('id')
		current = $("#tabc3 #license-type-2 option:selected").attr('current')
		title = $("#tabc3 #license-type-2 option:selected").attr('title')

		showFormCancel(requires_ip, requires_license_id, requires_domain, id, title, current);
}


function showFormCancel(requires_ip, requires_license_id, requires_domain, id, license, title)
{ 
	//$('#client-license-cancel .lc-box').slideUp();
	//$('#cancel-license').fadeIn();	
	$('#cancel-license #pack_id-2').val(id);
	...
	if (requires_license_id == 0)
	{
		$("#tabc3 #tr-license-id-2").css('display', 'none');	
		$("#tabc3 #license-id-2").removeClass("required");
		$('#tabc3 #license-id-2').val("");
			
	}
	else
	{
		$("#tabc3 #tr-license-id-2").css('display', '');	
		$("#tabc3 #license-id-2").addClass("required");
		$('#tabc3 #license-id-2').val(title);
	}
	if (requires_ip == 0)
	{
		$("#tabc3 #tr-current-ip-2").css('display', 'none');		
		$("#tabc3 #current-ip-2").removeClass("required");		
		$("#tabc3 #current-ip-2").removeClass("lc-required-missing");		
		$("#tabc3 #current-ip-2").removeClass("ip-address");		
		$('#tabc3 #current-ip-2').val("");
		
	}
	else
	{
		$("#tabc3 #tr-current-ip-2").css('display', '');		
		$("#tabc3 #current-ip-2").addClass("required");		
		$('#tabc3 #current-ip-2').val(title);
	...
	{
		$("#tabc3 #tr-current-domain-2").css('display', 'none');			
		$("#tabc3 #current-domain-2").removeClass("required");		
		$('#tabc3 #current-domain-2').val("");
	}
	else
	{
		$("#tabc3 #tr-current-domain-2").css('display', '');		
		$("#tabc3 #current-domain-2").addClass("required");		
		$('#tabc3 #current-domain-2').val(title);		
	}
